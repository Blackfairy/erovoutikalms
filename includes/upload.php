...
class  Media {
  ...
  public $trainingPath = SITE_ROOT.DS.'..'.DS.'uploads/trainings';
...
/*--------------------------------------------------------------*/
  ...
  /*--------------------------------------------------------------*/
  /* Function for Process training media file
  /*--------------------------------------------------------------*/
  public function process_training_media(){
    if(!empty($this->errors)){
        return false;
      }
    if(empty($this->fileName) || empty($this->fileTempPath)){
    ...
    if(!is_writable($this->trainingPath)){
        $this->errors[] = $this->trainingPath." Must be writable!!!.";
        return false;
      }

    if(file_exists($this->trainingPath."/".$this->fileName)){
      $this->errors[] = "The file {$this->fileName} already exists.";
      return false;
    }

    if(move_uploaded_file($this->fileTempPath, $this->trainingPath.'/'.$this->fileName))
    {

      if($this->insert_training_media()){ // Update the method name
        unset($this->fileTempPath);
        return true;
      }
  ...
  /*--------------------------------------------------------------*/
  /* Function for Insert training media image
  /*--------------------------------------------------------------*/
  private function insert_training_media(){ // Update the method name

    global $db;
    $sql  = "INSERT INTO training_media (file_name, file_type)"; // Update the table name
    $sql .=" VALUES ";
    $sql .="(
              '{$db->escape($this->fileName)}',
              '{$db->escape($this->fileType)}'
